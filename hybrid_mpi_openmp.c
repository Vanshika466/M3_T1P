...
#define N 8  // Test with small size first
...
MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    // Validate matrix division
    if (N % size != 0) {
        if (rank == 0) {
            fprintf(stderr, "Matrix size %d must be divisible by number of proc>
        }
        MPI_Finalize();
        return 1;
    }
...
MPI_Scatter(A, rows_per_process*N, MPI_DOUBLE,
    ...
    // OpenMP parallelized computation
